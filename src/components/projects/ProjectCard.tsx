...
export default function ProjectCard({ project, featured }: Props) {
  if (featured && project.stats.length > 0) {
    return (
      <Link
        ...
        className="group block md:col-span-2 bg-ink text-surface rounded-lg p-8 md:p-12 hover:-translate-y-1 transition-transform duration-200"
      >
        <div className="flex flex-wrap gap-2 mb-6">
          <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-accent-warm/20 text-accent-warm border border-accent-warm/30">
            {project.badge}
          </span>
        ...
        <h3
          className="text-2xl md:text-3xl font-bold mb-4 group-hover:text-accent-warm transition-colors"
  ...
  return (
    <Link
      href={`/projects/${project.slug}`}
      className="group block bg-surface border border-ink/10 rounded-lg p-6 hover:-translate-y-0.5 hover:border-ink/20 transition-all duration-200"
    >
      <div className="mb-4">
        <Badge variant="cool">{project.badge}</Badge>
      </div>
      <h3
        className="text-lg font-bold text-ink mb-3 group-hover:text-accent-cool transition-colors"
        style={{ fontFamily: 'var(--font-syne)' }}
      >
        {project.title}
      </h3>
